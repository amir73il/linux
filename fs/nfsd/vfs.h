...
static inline int fh_want_write_name(struct svc_fh *fh, char *fname, int flen)
{
	 ...
	 * may cause loosing pre_modify event on path/name.
	 */
				       ...
				       struct svc_fh *tfh, char *tname, int tlen)
{
	 ...
	 * cause loosing pre_modify event on source and target path/name.
	 */
	...
	ret = mnt_want_write_rename(&oldpath, &oldname, &newpath, &newname);
	if (!ret)

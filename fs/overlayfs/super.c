...
static bool ovl_dentry_is_stale(struct dentry *d, u64 hash)
{
	struct inode *inode = READ_ONCE(d->d_inode);

	return unlikely(!inode || IS_DEADDIR(inode) ||
			(hash && hash != d->d_name.hash_len));
}

static int ovl_revalidate_real(struct dentry *d, unsigned int flags, bool weak, u64 hash)
{
	int ret = 1;

	/* Invalidate dentry if real was deleted/renamed since we found it */
	if (ovl_dentry_is_stale(d, hash)) {
		ret = 0;
	} else if (weak) {
					...
					unsigned int flags, bool weak)
{
	...
	for (i = 0; ret > 0 && i < oe->numlower; i++, lower++)
		ret = ovl_revalidate_real(lower->dentry, flags, weak, lower->hash);
...
static int ovl_parse_redirect_mode(struct ovl_config *config, const char *mode)
{
	if (strcmp(mode, "on") == 0) {
		...
		/* Do not follow to lower dir on 'origin' mismatch */
		config->redirect_dir = true;
		config->redirect_follow = true;
			 ...
			 struct ovl_layer *upper_layer, struct path *upperpath)
{
	...
	if (ofs->config.redirect_origin) {
		/* Avoid "lowerdir in-use" warning from overlay snapshot mounts */
		return 0;
	} else if (ovl_inuse_trylock(ovl_upper_mnt(ofs)->mnt_root)) {
	...
	if (ofs->indexdir) {
		...
		if (ovl_check_origin_xattr(ofs->indexdir)) {
			err = ovl_verify_set_fh(ofs->indexdir, OVL_XATTR_ORIGIN,
						upperpath->dentry, true, false, true);
			if (err)

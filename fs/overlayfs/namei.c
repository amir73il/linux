				  ...
				  bool connected, bool nested)
{
	...
	if (nested) {
		...
		type = OVL_FILEID_V1;
		bytes = OVL_FH_LEN(fh);
	} else if (uuid_equal(&fh->fb.uuid, &mnt->mnt_sb->s_uuid)) {
		fid = fh->fb.fid;
		type = fh->fb.type;
	...
	real = exportfs_decode_fh(mnt, fid, bytes >> 2, type,
			    ...
			    struct ovl_lookup_data *d)
{
	...
	if (d->is_dir) {
		...
		pr_debug("implicit redirect dir (%pd2 -> %s) %s(idx=%d, err=%i)\n",
			 upperdentry, redirect, err ? "" : "set explicit ",
...
int ovl_verify_index(struct ovl_fs *ofs, struct dentry *index)
{
	...
	if (d_is_dir(index) && ofs->config.redirect_origin &&
	    ofs->config.nfs_export == OVL_NFS_EXPORT_NESTED) {
		int ctr = 0;
		struct ovl_lookup_data d = {
			.is_dir = true,
			.redirect = NULL,
	...
	if (!d.stop && upperdentry && !ctr && ofs->config.redirect_origin) {
		...
		if (d.opaque) {
			...
			if (stack[0].layer->idx < roe->numlower) {
				err = -ESTALE;
				pr_warn_ratelimited("retry lookup for mismatch origin (%pd2, layer=%d, redirect=%s)\n",
						    upperdentry,
						    stack[0].layer->idx,
						    d.redirect);
				goto out_put;

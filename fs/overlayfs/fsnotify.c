						...
						struct dentry *lowerdir)
{
	...
	struct dentry *indexdir = ovl_indexdir(sb);
	...
	index = ovl_lookup_lower_unlocked(sb, indexdir, &name);
	if (IS_ERR(index)) {
		err = PTR_ERR(index);
		goto fail;
	}
 ...
 *
 * If @dir is NULL, only test is index needs to be created.
 *
 * Returns >0 if lower dir index was created.
 * Returns  0 if no need to create index.
					    ...
					    struct dentry *lowerdir)
{
	...
	if (IS_ERR_OR_NULL(index)) {
		...
		/* With NULL @dir return 1 for create test */
		if (dir)
			err = ovl_do_mkdir(ofs, dir, index, S_IFDIR);
		if (!err || err == -EEXIST)
			err = 1;
	}
	dput(index);
	...
	pr_debug("%s: index of %pd2 %s\n", __func__, lowerdir,
		 err > 0 ? (dir ? "created" : "not found") :
		 (!err ? (index ? "exists" : "new") : "failed"));
				 ...
				 struct dentry *lowerdir)
{
	...
	if (IS_ERR_OR_NULL(index)) {
		 ...
		 */
		bool fsync = (mask & FS_MODIFY_INTENT) && !(mask & FS_ISDIR);
				 ...
				 struct dentry *lowerdir, u32 mask)
{
	...
	struct ovl_mark *ovm;
	int add_flags = FSNOTIFY_ADD_MARK_UPDATE_MASKS;
	int err = -ENOMEM;
			   ...
			   struct vfsmount *lowermnt, bool watch_mnt)
{
	...
	err = fsnotify_add_mark(&ovm->fsn_mark, obj, type, 0);
	fsnotify_put_mark(&ovm->fsn_mark);
...
int ovl_get_watch(struct super_block *sb, struct ovl_fs *ofs, struct path *lowerpath)
{
	...
	group = fsnotify_alloc_group(&ovl_fsnotify_ops, 0);
	if (IS_ERR(group)) {

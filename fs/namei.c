...
static int filename_parentat(int dfd, struct filename *name,
			     ...
			     struct lookup_result *res)
{
	return __filename_parentat(dfd, name, flags, parent, res, NULL);
}
...
static struct dentry *__kern_path_locked(struct filename *name, struct path *path)
{
	...
	d = lookup_one_qstr_excl(&res.last, path->dentry, 0);
	if (IS_ERR(d)) {
...
int vfs_path_parent_lookup(struct filename *filename, unsigned int flags,
			   ...
			   const struct path *root)
{
	return  __filename_parentat(AT_FDCWD, filename, flags, parent, res,
				    root);
}
					   ...
					   unsigned int lookup_flags, int *pidx)
{
	...
	if (res.last.name[res.last.len] && !want_dir)
		create_flags = 0;
	inode_lock_nested(path->dentry->d_inode, I_MUTEX_PARENT);
	dentry = lookup_one_qstr_excl(&res.last, path->dentry,
...
retry:
	...
	inode_lock_nested(path.dentry->d_inode, I_MUTEX_PARENT);
	dentry = lookup_one_qstr_excl(&res.last, path.dentry, lookup_flags);
	error = PTR_ERR(dentry);
	if (IS_ERR(dentry))
		goto exit3;
	if (!dentry->d_inode) {
...
retry_deleg:
	...
	old_dentry = lookup_one_qstr_excl(&old_res.last, old_path.dentry,
					  lookup_flags);

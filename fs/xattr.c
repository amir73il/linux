...
int
vfs_setxattr_notify(struct vfsmount *mnt, struct user_namespace *mnt_userns,
		    struct dentry *dentry, const char *name, const void *value,
		    size_t size, int flags)
{
	int error = vfs_setxattr(mnt_userns, dentry, name, value, size, flags);

	if (!error)
		fsnotify_xattr(mnt, dentry);
...
int
vfs_removexattr_notify(struct vfsmount *mnt, struct user_namespace *mnt_userns,
		       struct dentry *dentry, const char *name)
{
	int error = vfs_removexattr(mnt_userns, dentry, name);
	 ...
	 size_t size, int flags)
{
	...
	error = vfs_setxattr_notify(path->mnt, mnt_userns, path->dentry, kname,
				    kvalue, size, flags);
out:
	kvfree(kvalue);
...
removexattr(struct path *path, const char __user *name)
{
	...
	return vfs_removexattr_notify(path->mnt, mnt_user_ns(path->mnt),
				      path->dentry, kname);
}

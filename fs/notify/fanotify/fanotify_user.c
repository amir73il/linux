					     ...
					     __u32 mask, unsigned int fan_flags)
{
	...
	mask |= fsn_mark->mask;
	if (mask & FAN_ONDIR &&
	    fanotify_test_fs_event_mask(fsn_mark->group, mask,
			     ...
			     struct fan_fsid *fsid)
{
	 ...
	 */
	if (fanotify_test_fs_event_mask(group, mask, FAN_FS_ERROR) &&
...
out:
	...
	if (!ret && fanotify_test_fs_event_mask(group, mask,
				     ...
				     unsigned int flags)
{
	...
	bool strict_dir_events = FAN_GROUP_FLAG(group, FAN_REPORT_TARGET_FID) ||
			 fanotify_test_fs_event_mask(group, mask, FAN_RENAME) ||
	 ...
	 */
	if (fanotify_test_fs_event_mask(group, mask,
					FANOTIFY_PRE_CONTENT_EVENTS)) {
	 ...
	 */
	if (fanotify_test_fs_event_mask(group, mask, FANOTIFY_PERM_EVENTS) &&
	...
	if (strict_dir_events && mark_type == FAN_MARK_INODE && !is_dir &&
	    fanotify_test_fs_event_mask(group, mask,
...
static bool fanotify_is_valid_mask(struct fsnotify_group *group, __u64 mask)
{
	u32 valid_mask = 0;
			    ...
			    int dfd, const char  __user *pathname)
{
	...
	if (group->type != group_type ||
	    !fanotify_is_valid_mask(group, mask))
		return -EINVAL;

	/* Mount events require FAN_REPORT_MNT */
	if (fanotify_test_ns_event_mask(group, mask, FANOTIFY_MOUNT_EVENTS) &&
	...
	if (fanotify_test_fs_event_mask(group, mask, FANOTIFY_PERM_EVENTS) &&
	    group->priority == FSNOTIFY_PRIO_NORMAL)
		return -EINVAL;
	else if (fanotify_test_fs_event_mask(group, mask,
					     FANOTIFY_PRE_CONTENT_EVENTS) &&
		 group->priority == FSNOTIFY_PRIO_CONTENT)
		return -EINVAL;
	 ...
	 */
	if (fanotify_test_fs_event_mask(group, mask, FAN_RENAME) &&

				 ...
				 struct fanotify_event *event)
{
	...
	if (info_mode & FAN_REPORT_ACCESS_RANGE &&
	    fanotify_event_has_access_range(event))
				   ...
				   size_t info_len)
{
	...
	pr_debug("%s: group=%p fd=%d response=%u errno=%u buf=%p size=%zu\n", __func__,
		 group, fd, response, errno, info, info_len);
	/*
	...
	list_for_each_entry(event, &group->fanotify_data.access_list,
		...
		finish_permission_event(group, event, response, errno, &friar);
				      ...
				      char __user *buf, int count)
{
	...
	if (WARN_ON_ONCE(!pevent->range || !pevent->range->ppos))
		return 0;

	info.hdr.info_type = FAN_EVENT_INFO_TYPE_RANGE;
	info.hdr.len = info_len;
	info.offset = *(pevent->range->ppos);
	info.count = pevent->range->count;
		...
		if (!fanotify_is_perm_event(event->mask)) {
			...
			if (ret <= 0) {
				...
				finish_permission_event(group,
					FANOTIFY_PERM(event), FAN_DENY, 0, NULL);
				wake_up(&group->fanotify_data.access_waitq);
			} else {
				spin_lock(&group->notification_lock);
...
static ssize_t fanotify_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	struct fsnotify_group *group;
	int ret;
	const char __user *info_buf = buf + sizeof(struct fanotify_response_error);
	size_t info_len;
...
static int fanotify_release(struct inode *ignored, struct file *file)
{
	...
	while (!list_empty(&group->fanotify_data.access_list)) {
		...
		list_del_init(&event->fae.fse.list);
		finish_permission_event(group, event, FAN_ALLOW, 0, NULL);
		spin_lock(&group->notification_lock);
	}
			    ...
			    int dfd, const char  __user *pathname)
{
	...
	/*
	 * FAN_CLASS_NOTIF groups are not allowed to request permission events.
	 */
	ret = -EINVAL;
	if (mask & FANOTIFY_PERM_EVENTS &&
	    FAN_GROUP_CLASS(group) == FAN_CLASS_NOTIF)
		goto fput_and_out;

...
/*
 * FAN_REPORT_FID is ambigious in that it reports the fid of the child for
 * some events and the fid of the parent for dirent events.
 *
 * With flag combination FAN_REPORT_DIR_FID|FAN_REPORT_FID, in addition to
 * reporting FAN_EVENT_INFO_TYPE_DFID info record on events on non-dir child,
 * the fid of the child is reported with info type FAN_EVENT_INFO_TYPE_FID.
 *
 * With the additional flag FAN_REPORT_FID_OF (i.e. FAN_REPORT_ALL_FIDS), the
 * FAN_EVENT_INFO_TYPE_FID info record with child fid is reported also on dirent
 * events, regardless if the child is a subdir or non-dir.
 */
static inline bool fanotify_report_child_fid(unsigned int fid_mode, u32 mask)
{
	if (mask & ALL_FSNOTIFY_DIRENT_EVENTS)
		return (fid_mode & FAN_REPORT_FID_OF);
	else
 ...
 * and the group flags.
 *
 * With the group flag FAN_REPORT_FID_OF, always report the child fid.
 *
 * Without the group flag FAN_REPORT_FID_OF, report the modified directory fid
 * on dirent events and the the child fid otherwise.
 *
 * For example:
 * FS_ATTRIB reports the child fid even if reported on a watched parent.
 * FS_CREATE reports the modified dir fid without FAN_REPORT_FID_OF
 *       and reports the created child fid with FAN_REPORT_FID_OF.
 */

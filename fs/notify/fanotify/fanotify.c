...
{
	if (mask & FANOTIFY_DIRONLY_EVENTS)
...
{
	if ((event_mask & FANOTIFY_DIRONLY_EVENTS) &&
				...
				__kernel_fsid_t *fsid, u32 match_mask)
{
	...
	if ((fid_mode & FAN_REPORT_DIR_FID) && dirid) {
		...
		if (!(fid_mode & FAN_REPORT_NAME)) {
			name_event = !!child;
			file_name = NULL;
		} else if ((mask & FANOTIFY_DIRONLY_EVENTS) || !ondir) {
			name_event = true;
		}
				 ...
				 struct fsnotify_iter_info *iter_info)
{
	...
	BUILD_BUG_ON(FAN_PRE_DIR_ACCESS != FS_PRE_DIR_ACCESS);

...
static struct fanotify_event *fanotify_alloc_fs_event(
				struct fsnotify_group *group,
				...
				unsigned int *hash)
{
	...
	if (WARN_ON_ONCE(mask & FS_EVENT_ON_NS))
		return NULL;
...
static struct fanotify_event *fanotify_alloc_ns_event(
				...
				unsigned int *hash)
{
	...
	if (mnt_id) {
		...
		*hash ^= hash_long(mnt_id, FANOTIFY_EVENT_HASH_BITS);
	} else {
		WARN_ON_ONCE(1);
	}
				...
				__kernel_fsid_t *fsid, u32 match_mask)
{
	...
	if (fsnotify_is_ns_watcher(group)) {
		event = fanotify_alloc_ns_event(group, mask, data, data_type,
						&hash);
	} else {
		event = fanotify_alloc_fs_event(group, mask, data, data_type,
						dir, name, fsid, match_mask,
						&hash);
	}
	if (!event)
		return NULL;
				 ...
				 struct fsnotify_iter_info *iter_info)
{
	...
	pr_debug("%s: group=%p type=%d mask=%llx report_mask=%x\n", __func__,

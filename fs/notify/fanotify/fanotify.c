							...
							gfp_t gfp)
{
	...
	pevent->range = fsnotify_data_file_range(data, data_type);
	pevent->path = *path;
	path_get(path);
				 ...
				 struct fsnotify_iter_info *iter_info)
{
	...
	BUILD_BUG_ON(FAN_PRE_ATTRIB != FS_PRE_ATTRIB);
	BUILD_BUG_ON(FAN_PRE_CREATE != FS_PRE_CREATE);
	BUILD_BUG_ON(FAN_PRE_DELETE != FS_PRE_DELETE);
	BUILD_BUG_ON(FAN_PRE_MOVE_TO != FS_PRE_MOVE_TO);
	BUILD_BUG_ON(FAN_PRE_MOVE_FROM != FS_PRE_MOVE_FROM);

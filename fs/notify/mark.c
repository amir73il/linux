 ...
 *
 * When it's time to drop the proxy refcount, clear the WANT_IREF flag
 ...
 * is detached.
 *
 * Note that the proxy refcount is NOT dropped if backend only removed the
 * WANT_IREF mark flag and does detach the mark!
 */
...
static void *__fsnotify_recalc_mask(struct fsnotify_mark_connector *conn)
{
	...
	hlist_for_each_entry(mark, &conn->list, obj_list) {
		...
		if (mark->flags & FSNOTIFY_MARK_FLAG_WANT_IREF)
			want_iref = true;
	}
			     ...
			     void **prealloc_conn)
{
	...
	if (obj_type == FSNOTIFY_OBJ_TYPE_INODE &&
		...
		mark->flags |= FSNOTIFY_MARK_FLAG_WANT_IREF;

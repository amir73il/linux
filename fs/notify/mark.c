 ...
 * to pre-modify permission events on any sb objects.
 */
static void fsnotify_update_sb_watchers(struct fsnotify_mark_connector *conn,
					u32 old_mask, u32 new_mask)
{
	struct super_block *sb = fsnotify_connector_sb(conn);
	bool old_watchers = old_mask & FSNOTIFY_PRE_MODIFY_EVENTS;
	bool new_watchers = new_mask & FSNOTIFY_PRE_MODIFY_EVENTS;

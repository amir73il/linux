...
static __u32 *fsnotify_conn_mask_p(struct fsnotify_mark_connector *conn)
{
	...
	else if (conn->type == FSNOTIFY_OBJ_TYPE_SUBTREE)
		return &fsnotify_conn_subtree_mark(conn)->fsn_mark.mask;
					...
					unsigned int *type)
{
	...
	*type = conn->type;
	...
	if (conn->type == FSNOTIFY_OBJ_TYPE_INODE) {
		...
		fsnotify_conn_subtree_mark(conn)->fsn_mark.mask = 0;
	} else if (conn->type == FSNOTIFY_OBJ_TYPE_SB) {
					   ...
					   unsigned int domain)
{
	...
	hlist_for_each_entry(mark, &conn->list, obj_list) {
		if (mark->group == group &&
		    FSNOTIFY_MARK_DOMAIN(mark) == domain &&
		    (mark->flags & FSNOTIFY_MARK_FLAG_ATTACHED)) {
...
/* Clear any marks in a group with given type mask and domain */
void __fsnotify_clear_marks_by_group(struct fsnotify_group *group,
				     unsigned int type_mask, unsigned int domain)
{
	struct fsnotify_mark *lmark, *mark;
	LIST_HEAD(to_free);
	struct list_head *head = &to_free;

	/* Skip selection step and ignore domain if we want to clear all marks. */

...
{
	return fsnotify_file(file, FS_MODIFY_INTENT);
}
...
{
	return fsnotify_parent(path->dentry, FS_MODIFY_INTENT, path,
			       FSNOTIFY_EVENT_PATH);
}
...
{
	return fsnotify_name(FS_NAME_INTENT, path, FSNOTIFY_EVENT_PATH,
 ...
 * If exchanged, backends will get a FS_MOVE_INTENT for new_path/new_name.
 * Otherwise, backends will get a FS_NAME_INTENT events that looks like a
 * link/unlink of new_path/new_name.  The only difference for backends
 * from an event generated by link()/unlink() is the rename cookie.
 */
					     ...
					     unsigned int flags)
{
	...
	u32 new_mask = FS_NAME_INTENT;
	int ret;

	ret = fsnotify_name(FS_MOVE_INTENT, old_path, FSNOTIFY_EVENT_PATH,
			    d_inode(old_path->dentry), old_name, cookie);
	if (ret)
		return ret;

	if (flags & RENAME_EXCHANGE)
		new_mask = FS_MOVE_INTENT;

	return fsnotify_name(new_mask, new_path, FSNOTIFY_EVENT_PATH,
